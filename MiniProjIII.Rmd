...
#Cai's slope option
trails_avg_slope <- trails_pt %>%
  group_by(name) %>%
  summarize(avg_segment_slope = sd(ELEV_M * ), N = (n() - 1), len = mean(SHAPE_LEN)) %>%
  mutate(tot_slope_change = N * avg_segment_slope, avg_m_slope = avg_segment_slope / len)
...
Our_map <- leaflet(data = c( trails_inbound)) %>%
  #addTiles() %>% #We know how to add the background but I didn't like the aesthetic....
  addPolygons(data = macleish_layers[["boundary"]], weight = 3, color = "#01561d", opacity = 0.85)%>%
  ...
  addPolylines(data = elev, weight = 0.78, color = "#011c0a")
