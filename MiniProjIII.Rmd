...
map_T_ev <- leaflet() %>%
  addTiles() %>%
  addPolylines(data = macleish_layers[["trails"]], weight = 2, popup = ~name, color = "red") %>%
  addPolylines(data = macleish_layers[["boundary"]], weight = 3, color = "blue") %>%
...
noho_elev %>%
  ...
  mutate(ELEV_CHANGE = ELEV_M/SHAPE_LEN) #elevation change to indicate a steepness over the length of the trail
```
