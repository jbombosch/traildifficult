...
trails <- macleish_layers[["trails"]] %>%
  ...
  group_by(name) %>%
  summarize(total_length_mile = sum(length)/1609.34) #currently doesn't have units, but does it need units to be used later?
...
df_trails2 <- as.data.frame(trails_temp)%>%
  select(name, slope, len, color, geometry)%>%
  filter(slope>0)
...
df <- df_trails %>%
  ...
  mutate(total_length_mile = as.numeric(len), 
        avg_slope_m = as.numeric(avg_slope_m))

df <- df %>%
  mutate(diff_lvl = sqrt(2*avg_slope_m*len)) %>%
  arrange(name)
#df_diffindex_trail <- lapply(diff_index, FUN = NEED A FUNCTION TO ORDER )
>>>>>>> c2b15704f0e7f174e7105fc748143f7e11b4bc34
...
```{r}
easy_length <- trails %>%
  filter(total_length_mile < 1) #total_length mile is not showing up as something with untis

moderate_length <- trails %>%
  filter(total_length_mile > 1, total_length_mile < 5)
...
Our_map <- leaflet(data = c( trails_inbound)) %>%
  ...
  addPolylines(data = trails, weight = 2, popup = ~name, color = "#4c00ff") %>%# here is where we would start adding our trails by difficulty average. :)
  addPolylines(data = elev, weight = 0.78, color = "#004400")%>%
  addPolygons(data = buildings, color = "202120", stroke = FALSE, fillOpacity = 0.65, weight = 1, popup = ~name, group = "Buildings and Landmarks")%>%
  ...
  addMarkers(data = camp_neu, 
    ...
    addPolygons(data = macleish_layers[["forests"]], 
              ...
              fillOpacity = 0.2,
              popup = ~ Sheet1__Na, group = "Forests and Streams", color = "#184228") %>%
  addPolygons(data = macleish_layers[["wetlands"]], 
              weight = 1, group = "Forests and Streams", color = "#0f592c")%>%
    addPolylines(data = water, 
               weight = 2, group = "Forests and Streams")%>%
  addLayersControl(
    overlayGroups = c( "Buildings and Landmarks", "Forests and Streams"), # we will add polylines for the level of difficulty then we can group by category.
