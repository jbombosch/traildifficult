...
>>>>>>> dbc5685d83e455ebf35406bccc1a627c5c1fe179
...
temp <- data.frame(trails_inbound) %>%
  group_by(name) %>%
  mutate(slope = abs(c(NA, diff(ELEV_M))), len = abs(c(NA, diff(SHAPE_LEN)))) %>%
  filter(name != "Easy Out", !is.na(slope)) %>%
  select(name, SHAPE_LEN, ELEV_M, slope, len) %>%
  mutate(slope_m = slope / len) %>%
  summarize(avg_slope_m = mean(slope_m), total_len = sum(SHAPE_LEN), tot_change = sum(slope), lvl = avg_slope_m * 20)
...
hardsegments <- trails_slope %>%
  filter(lvl > 20)

moderatesegments <- trails_slope %>%
  filter(lvl < 20, avg_slope_m > 2)

easysegments <- trails_slope %>%
  filter(lvl < 2)
