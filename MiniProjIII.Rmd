...
hard <- trails_slope_sf %>%
  filter(avg_slope_m > 20) #define difficulty

moderate <- trails_slope_sf %>%
  filter(avg_slope_m < 20, avg_slope_m > 2)

easy <- trails_slope_sf %>%
  filter(avg_slope_m < 2)

hardsegments <- df %>%
  filter(lvl > 20) #define difficulty
...
Our_map <- leaflet(data = c( trails_inbound)) %>%
  ...
  addCircleMarkers(data = st_cast(trails_elev, "POINT"), color = "purple",radius = 2, popup = ~paste("Trail:",name), group = "Trail Segments") %>%
 addPolylines(data = trails, weight = 2, popup = ~name, color = ~meepmeep(trails$name)) %>%# here is where we would start adding our trails by difficulty average. :)
  ...
  addPolygons(data = buildings, color = "202120", stroke = FALSE, fillOpacity = 1, weight = 1, popup = ~name, group = "Buildings and Landmarks")%>%
